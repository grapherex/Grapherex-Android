...
public class ContactSelectionListItem extends LinearLayout implements RecipientForeverObserver {
    ...
    public void set(@NonNull GlideRequests glideRequests,
                    ...
                    IOnAudioCallClicked callClicked,
                    boolean hidePhoneCallIcon) {
        ...
        if (recipient != null && !recipient.get().isSelf()) {
            ivCall.setOnClickListener(v -> {
                if (callClicked != null) {
                    callClicked.onCallClick(recipient.get());
                }
            });
        }
        ivCall.setVisibility(hidePhoneCallIcon ? INVISIBLE : VISIBLE);
    }
    ...
    public interface IOnAudioCallClicked {
        void onCallClick(Recipient recipient);
    }
}
