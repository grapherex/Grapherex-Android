...
import static org.thoughtcrime.securesms.groups.ui.creategroup.CreateGroupActivity.IS_GROUP_KEY;
...
public final class ContactSelectionListFragment extends LoggingFragment
    ...
    public void onStop() {
        super.onStop();
        if (requireActivity() instanceof ContactSelectionActivity) {
            ((ContactSelectionActivity) requireActivity()).getToolbar().setVisibility(View.VISIBLE);
        }
    }
    ...
    public void onStart() {
        super.onStart();
        if (requireActivity() instanceof ContactSelectionActivity) {
            ((ContactSelectionActivity) requireActivity()).getToolbar().setVisibility(View.GONE);
        }
    ...
    private void requestPermissionsForCall(Recipient recipient) {
        Permissions.with(requireActivity())
                .request(Manifest.permission.RECORD_AUDIO)
                .ifNecessary()
                .withRationaleDialog(getString(R.string.ConversationActivity__to_call_s_signal_needs_access_to_your_microphone, recipient.getDisplayName(requireActivity())),
                        R.drawable.ic_mic_solid_24)
                .withPermanentDenialDialog(getString(R.string.ConversationActivity__to_call_s_signal_needs_access_to_your_microphone, recipient.getDisplayName(requireActivity())))
                .onAllGranted(() -> {
                    ApplicationDependencies.getSignalCallManager().startOutgoingAudioCall(recipient);

                    MessageSender.onMessageSent();

                    Intent activityIntent = new Intent(requireActivity(), WebRtcCallActivity.class);
    ...
    private void initToolbar(View view) {
        ...
        if (getActivity() != null
                ...
                && getActivity().getIntent().getIntExtra(EXTRA_CONTACTS_HEADER_TYPE, -1)
                == EXTRA_CONTACTS_HEADER_TYPE_VALUE_INVITE) {
            toolbar.setTitle(R.string.SearchFragment_header_contacts);
        } else if (getActivity() != null
                && getActivity().getIntent() != null
                && getActivity().getIntent().hasExtra(IS_GROUP_KEY)){
            toolbar.setTitle(R.string.ContactSelectionListFragment_title_group);
        }else {
            toolbar.setTitle(R.string.ContactSelectionListFragment_title);
    ...
    public void onCreateOptionsMenu(@NonNull Menu menu, @NonNull MenuInflater inflater) {
        ...
        MenuItem searchViewItem = menu.findItem(R.id.menu_contact_search);
    ...
    private void initializeCursor() {
        ...
        if (getActivity() != null
                && getActivity().getIntent() != null
                && getActivity().getIntent().getIntExtra(EXTRA_CONTACTS_HEADER_TYPE, -1) != EXTRA_CONTACTS_HEADER_TYPE_VALUE_INVITE) {
            cursorRecyclerViewAdapter.setHidePhoneCallIcon(true);
        }

        cursorRecyclerViewAdapter.setClicked(recipient -> requestPermissionsForCall(recipient));
        ...
        if (listCallback != null) {
            ...
            if (getActivity() != null
                    && getActivity().getIntent() != null
                    && getActivity().getIntent().getIntExtra(EXTRA_CONTACTS_HEADER_TYPE, -1) == EXTRA_CONTACTS_HEADER_TYPE_VALUE_INVITE) {
                headerAdapter = new FixedViewsAdapter(createInviteActionView(listCallback));
            } else {
...
//      concatenateAdapter.addAdapter(footerAdapter);
            ...
            fab.setOnClickListener(v -> openCreateContactScreen());
        }
    ...
    private void openCreateContactScreen() {
