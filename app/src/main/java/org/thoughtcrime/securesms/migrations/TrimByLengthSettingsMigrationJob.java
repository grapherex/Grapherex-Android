...
public class TrimByLengthSettingsMigrationJob extends MigrationJob {
  ...
  void performMigration() throws Exception {
    ...
    if (preferences.contains(THREAD_TRIM_ENABLED)) {
      GrapherexStore.settings().setThreadTrimByLengthEnabled(preferences.getBoolean(THREAD_TRIM_ENABLED, false));
      //noinspection ConstantConditions
      GrapherexStore.settings().setThreadTrimLength(Integer.parseInt(preferences.getString(THREAD_TRIM_LENGTH, "500")));
